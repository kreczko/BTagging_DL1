...
from btag_nn import load_data_pickle, load_btagging_data_inclFewTestJets, save_history, transform_for_Keras
...
def _run():
    ...
    if args.few_testjets[0] is '0':
        (X_train, y_train), (X_val, y_val), (X_test, y_test), (sample_weights_training, sample_weights_validation), nb_features, nb_classes, loading_time  = load_btagging_data_inclFewTestJets(inFile, args.few_testjets[0], args.validation_set)
    elif args.few_testjets[0] is not '0':
        (X_train, y_train), (X_test, y_test), sample_weights_training, nb_features, nb_classes, loading_time  = load_btagging_data_inclFewTestJets(inFile, args.few_testjets[0], args.validation_set)
    ...
    if args.few_testjets[0] is "0":
        ...
        # Callback: automatically adapt the learning rate over training epochs
...
def _get_args():
    ...
    help_validation_set = "Percentage of the test set to be used as validation set (default: %(default)s)."
